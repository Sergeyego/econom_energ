...
ModelNorm::ModelNorm(QObject *parent) : DbTableModel("econom_energ_norm",parent)
{
    ...
    setSuffix("inner join diam d on d.id = econom_energ_norm.id_diam");
    setSort("d.diam, econom_energ_norm.id_spool");
...
{
    QSqlQuery query;
    query.prepare("insert into econom_energ_norm (id_diam, id_spool) "
                  "select "
                  "distinct m.id_diam, p.id_pack "
                  ...
                  "(m.id_diam || '-' || p.id_pack not in (select id_diam || '-' || id_spool from econom_energ_norm)) "
                  "order by m.id_diam, p.id_pack ");
